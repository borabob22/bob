...
import csv
with open(f"{path}events.tsv") as file:
	data = list(csv.reader(file, delimiter="\t"))
	cards = [{data[0][i]: line[i] for i in range(len(data[0]))} for line in data[1:]]

with open(f"{path}cards_template.svg") as file:
	data = file.read()
...
with open(f"{path}events_exported.svg", "w+") as file:
	print(re.sub(group_regex, card_groups, data), file=file)
